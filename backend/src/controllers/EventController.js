...
class EventController {
    ...
    getEvents = asyncHandler(async (req, res) => {
        ...
        const result = await this.eventService.getEventsForProfile(profileId, timezone, page, limit);

        sendSuccess(res, MESSAGES.EVENT.FETCHED, result, STATUS_CODES.SUCCESS);
    });

    getEventById = asyncHandler(async (req, res) => {
        const event = await this.eventService.getEventById(req.params.id, req.query.timezone);
    ...
    updateEvent = asyncHandler(async (req, res) => {
        ...
        const event = await this.eventService.updateEvent(id, body, updatedBy, timezone);
